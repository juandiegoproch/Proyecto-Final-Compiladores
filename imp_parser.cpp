...
Token* Scanner::nextToken() {
  ...
  startLexema();
  if (isdigit(c)) { // read a digit
    c = nextChar();
    while (isdigit(c)) c = nextChar();
    rollBack();
    token = new Token(Token::NUM, getLexema());
  } 
  else if (isalpha(c)) { // read an identifier/reserved
    c = nextChar();
  ...
  } else if (strchr("()+-*/;=<,!:", c)) { // operator-like
    switch(c) {
    ...
    case '/':
    {
      ...
      if (c == '/') 
      {
        // we are in a comment 
        /*
        comments are one line, so they end in either \n or \0 
        if they end in \0 they ammount to an END token. Else, the 
        next token follows the comment (suggesting recursion)
        */
        c = nextChar();
        while ( c != '\0' && c != '\n'){c = nextChar();} // consume the comment (and 1 more!)
        switch (c)
        {
        case '\n': // if it didnt contain the end, then the next token awaits!
          token = nextToken();
          cout << token;
          break;
        case '\0':
          token = new Token(Token::END); // otherwise, there is nothing we can do.
        }
      }
      else
      {
        rollBack(); // I looked ahead for comment detection
...
Stm* Parser::parseStatement() {
  ...
  else if (match(Token::DO)) //do-while
  {
    ...
    */
    Body* body = parseBody();
    if (!match(Token::WHILE)){ cout << "Do-While incompleto: no se encontro 'while' " << endl; exit(0);}
    Exp* c = parseCExp();
    s = new DoWhileStatement(c,body);
  }
  else if (match(Token::CONTINUE))
  {
    s = new LoopSkipStatement(LoopSkipType::CONTINUE);
  }
  else if (match(Token::BREAK))
  {
    s = new LoopSkipStatement(LoopSkipType::BREAK);
  }
  else {
