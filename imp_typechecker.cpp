...
void ImpTypeChecker::typecheck(Program* p) {
  env.clear();
  loopStack.clear(); // valido porque los objetos los limpia el AST
  p->accept(this);
  return;
}
...
void ImpTypeChecker::visit(DoWhileStatement* d)
{
  loopStack.push_back(d);
  d->body->accept(this); // body must be ok and
  ImpType etype = d->cond->accept(this);
  if (!etype.match(booltype)) {cout << "Condicional de do-while debe ser bool" << endl; exit(0);}
  loopStack.pop_back();
  return;
}

void ImpTypeChecker::visit(LoopSkipStatement* e)
{
  if (loopStack.empty())
  {
    cout << "Loop control flow statement used outside loop" << endl;
    exit(0);
  }
  return;
}
